...
func TestGetTokensInvalidPart(t *testing.T) {
    ...
    if filterPairs[0].filterType != "host" || filterPairs[0].filterContent != "google.com" { t.Errorf("filterType = \"%s\" && filterContent = \"%s\", expected host && google.com", filterPairs[0].filterType, filterPairs[0].filterContent)
    }
}
